...
java -jar applications/service/account-service/target/account-service-1.0.0.jar --server.port=8070 --spring.data.gemfire.pool.locators="localhost[10334]"
```
...
```shell
curl -X 'GET' \
  'http://localhost:8070/accounts/1' \
  -H 'accept: */*';echo
```
